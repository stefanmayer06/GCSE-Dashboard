...
export default function Lesson() {
  ...
  return <ScrollScreen contentContainerStyle={[styles.content, width >= 760 && styles.tablet]}>
    ...
    <Button variant="secondary" onPress={()=>setRecall(!recall)} accessibilityState={{expanded:recall}}>Before you read: what do you remember?</Button>
    {recall && <Notice title="Try retrieving first">Explain this idea in your own words. Name one step you know and one question you have, then read to fill the gap.</Notice>}
    {notes.length + extraBlocks.length > 0 ? <View style={styles.notes}><Text accessibilityRole="header" style={[styles.h2, { color: colors.ink }]}>Understand the idea</Text>{[...notes, ...extraBlocks].map((block, index) => <Note key={index} block={block} index={index}/>)}</View> : <Notice title="NOTES NOT PUBLISHED">This topic exists in the course index, but the server returned no structured lesson notes.</Notice>}
...
const styles = StyleSheet.create({content:{padding:20,paddingBottom:56,gap:20},tablet:{width:'100%',maxWidth:820,alignSelf:'center',paddingHorizontal:36},back:{fontFamily:'DMSans',fontWeight:'800',fontSize:11,letterSpacing:1},cache:{fontSize:13},statusLine:{flexDirection:'row',flexWrap:'wrap',gap:12},meta:{fontFamily:'DMSans',fontSize:11,fontWeight:'800',letterSpacing:.8},purpose:{borderTopWidth:2,borderBottomWidth:1,paddingVertical:18,gap:9},purposeText:{fontFamily:'Fraunces',fontSize:22,lineHeight:31},notes:{gap:16},h2:{fontFamily:'Fraunces',fontSize:27},paragraph:{fontSize:16,lineHeight:25},list:{gap:10},listRow:{flexDirection:'row',gap:12},bullet:{fontFamily:'DMSans',fontSize:11,fontWeight:'800',paddingTop:4},listText:{flex:1,fontSize:16,lineHeight:24},method:{borderWidth:1,borderLeftWidth:5,padding:16,gap:10},methodText:{fontFamily:'DMSans',fontSize:15,lineHeight:24},example:{borderWidth:1,padding:16,gap:10},exampleQ:{fontFamily:'Fraunces',fontSize:20,lineHeight:27,fontWeight:'700'},mistakes:{borderWidth:1,borderTopWidth:5,padding:16,gap:14},practice:{borderWidth:1,padding:18,gap:12},resources:{gap:8},resource:{paddingVertical:13,borderBottomWidth:1,gap:4},resourceTitle:{fontSize:16,fontWeight:'700'},tutor:{borderWidth:1,padding:18,gap:12}});
