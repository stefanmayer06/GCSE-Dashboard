...
export default function WeeklySummary() {
  ...
  return <ScrollScreen>
    ...
    {progress && <View style={{ borderWidth: 2, borderColor: tokens.accent, backgroundColor: colors.raised, padding: 18, gap: 8 }}><Text style={{ fontFamily: 'Fraunces', fontSize: 28, color: colors.ink }}>{subject === 'english' ? 'English' : subject === 'maths-higher' ? 'Maths Higher' : 'Maths Foundation'}</Text><Text style={{ color: colors.ink, fontSize: 17 }}>{progress.lessons} lessons / {progress.tests} tests / {progress.practiceAnswered} practice answers</Text><Text style={{ color: colors.quiet }}>Server-recorded accuracy: {progress.accuracy === null ? 'not measured' : `${progress.accuracy}%`} / streak: {progress.streak} days</Text></View>}
