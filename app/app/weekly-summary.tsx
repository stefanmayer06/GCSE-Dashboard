...
export default function WeeklySummary() {
  ...
  const query = useQuery({ queryKey: ['weekly-summary', subject], queryFn: () => client.progress() });
  ...
  return <ScrollScreen>
    ...
    <SectionHeader title="This week's notebook" meta={`${rows.length} SAVED`} />
    <Text style={{ color: colors.ink }}>{due} mistake reviews are currently due.</Text>
    {rows.slice(0, 5).map(row => <Text key={row.id} style={{ color: colors.quiet }}>- {row.topicName}: {row.prompt}</Text>)}
  </ScrollScreen>;
}