...
type Prefs = {
  ...
  setPlanning: (v: PlanningPreferences) => Promise<void>;
};
...
function PreferencesProvider({ children }: PropsWithChildren) {
  ...
  const [subject, setSubjectState] = useState<Subject>("maths");
  const [appearance, setAppearanceState] = useState<Appearance>("system");
  const prefsIdentity = useRef(`${userId}:${subject}`);
  useEffect(() => {
    prefsIdentity.current = `${userId}:${subject}`;
  }, [userId, subject]);
  const [planning, setPlanningState] = useState(defaultPlanningPreferences);
  const [hydrated, setHydrated] = useState(false);
  useEffect(() => {
    AsyncStorage.multiGet(["subject", "appearance"])
      .then((v) => {
        const s = v[0][1] as Subject | null;
        const a = v[1][1] as Appearance | null;
        if (s && ["maths", "maths-higher", "english"].includes(s))
          setSubjectState(s);
        if (a && ["system", "light", "dark"].includes(a)) setAppearanceState(a);
      })
      .catch(() => undefined)
  ...
  useEffect(() => {
    ...
    queryClient
      .fetchQuery({
        ...
        queryFn: () => hydratePersonal(client, userId, subject),
      })
      .then((value) => {
        if (active)
          setPlanningState(value.preferences ?? defaultPlanningPreferences);
      })
      .catch(() => {
        if (active) setPlanningState(defaultPlanningPreferences);
      });
  ...
  const setSubject = (v: Subject) => {
    setSubjectState(v);
    void AsyncStorage.setItem("subject", v);
  };
  const setAppearance = (v: Appearance) => {
    setAppearanceState(v);
    void AsyncStorage.setItem("appearance", v);
  };
  const setPlanning = async (v: PlanningPreferences) => {
    if (!userId) throw new Error("Sign in to save your preferences.");
    const savingIdentity = `${userId}:${subject}`;
    await new ApiClient(subject).savePreferences(v);
    if (prefsIdentity.current === savingIdentity) setPlanningState(v);
...
function NetworkProvider({ children }: PropsWithChildren) {
  const [status, setStatus] = useState({ online: false, hydrated: false });
  useEffect(() => {
    let active = true;
    ...
    const update = (state: Network.NetworkState) => {
      ...
      if (active) setStatus({ online, hydrated: true });
    };
    ...
    void Network.getNetworkStateAsync()
      .then((state) => {
        if (!listenerSeen) update(state);
      })
      .catch(() => {
        if (!listenerSeen) {
          onlineManager.setOnline(false);
          if (active) setStatus({ online: false, hydrated: true });
        }
      });
    return () => {
      active = false;
...
export function updateAuthIdentity(
  ...
  if (event === "SIGNED_OUT" || (previous !== undefined && previous !== next))
    clearCache();
  return next;
}
...
export function startAuthSession(
  ...
  const bootstrap = async () => {
    ...
    try {
      ...
      if (
        !userData.user &&
        (!userError || isConfirmedAuthRejection(userError))
      ) {
...
function AuthProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    const stopAuth = startAuthSession(client, (next, event) => {
      queryIdentity = updateAuthIdentity(queryIdentity, next, event, () =>
        queryClient.clear(),
      );
      setSession(next);
      setLoading(false);
