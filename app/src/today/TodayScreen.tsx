...
export function TodayScreen() {
  ...
  if (loading && !hasAllData) return <SafeAreaView style={[styles.screen, { backgroundColor: colors.paper }]}><PaperPattern/><View style={styles.content}><DeskHeader title="Make room for understanding." eyebrow="REVISION REGISTER"/><Skeleton height={44}/><Skeleton height={210}/><Skeleton height={92}/></View></SafeAreaView>;
  ...
  return <SafeAreaView style={[styles.screen, { backgroundColor: colors.paper }]}><PaperPattern/><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={subjectTheme.accent}/>}>
    ...
    {todayMission ? <View style={[styles.hero, todayMission.topicId ? { backgroundColor: colors.raised, borderColor: colors.ink, borderLeftColor: subjectTheme.accent } : { backgroundColor: colors.raised, borderColor: colors.ink, borderLeftColor: colors.warning }]}>
      <View style={styles.heroMeta}><Text style={[styles.mono, { color: todayMission.topicId ? subjectTheme.accent : colors.warning }]}>Your next session</Text><Text style={[styles.mono, { color: colors.quiet }]}>{todayMission.minutes} MIN / SAVED PLAN</Text></View>
      ...
      <View style={styles.heroMeta}><Text style={[styles.mono, { color: colors.positive }]}>Your next session COMPLETE</Text><Text style={[styles.mono, { color: colors.positive }]}>✓ {todayDone.task.toUpperCase()}</Text></View>
      ...
      <Text style={{ color: colors.quiet, lineHeight: 20 }}>Come back tomorrow. Your plan is saved to your account and the rest of the week stays locked until a new day starts.</Text>
    ...
    {progress.tests===0&&<Button variant="secondary" onPress={()=>router.push({pathname:'/practice',params:{diagnostic:'1'}})}>Find my starting point</Button>}
