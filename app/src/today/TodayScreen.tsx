...
export function TodayScreen() {
  ...
  return <SafeAreaView style={[styles.screen, { backgroundColor: colors.paper }]}><PaperPattern/><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={subjectTheme.accent}/>}>
    <View style={styles.topline}><View style={{ flex: 1 }}><DeskHeader title="Today at the desk" eyebrow="REVISION REGISTER"/></View><Link href="/settings" asChild><Pressable accessibilityRole="link" accessibilityLabel="Open profile and settings" style={[styles.settings, { borderColor: colors.strong }]}><Text style={[styles.mono, { color: colors.ink }]}>PROFILE</Text></Pressable></Link></View>
    ...
    <Link href="/settings" asChild><Pressable accessibilityRole="link" style={[styles.profile, { borderColor: colors.strong }]}><Text style={[styles.profileTitle, { color: colors.ink }]}>Profile and settings</Text><Text style={{ color: colors.quiet }}>Course, appearance and account &gt;</Text></Pressable></Link>
