...
export function TodayScreen() {
  ...
  const fadedCount = memriDue(mistakes.filter(row => row.subject === subject), todayNow).length;
  ...
  const fixupIds = subject === 'english'
    ? fixupEnglishPlan(dueMistakes(mistakes.filter(row => row.subject === subject), todayNow).map(row => row.topicId).filter((id): id is string => !!id)).skillIds
    : fixupTargets(focus.map(t => ({ id: t.id, accuracy: t.accuracy, answered: t.answered })), mistakes.filter(row => row.subject === subject));

  async function shareMilestone(id: string, label: string) {
    const message = `I just earned "${label}" revising on GCSE Study Desk — ${progress.tests} timed papers, ${progress.streak}-day streak and counting. Free AQA practice with worked solutions and a mistake notebook that brings misses back until they stick.`;
    try {
      await Share.share({ message, title: 'GCSE Study Desk milestone' });
      new ApiClient(subject).trackEvent('milestone_shared', { milestone: id });
    } catch { /* dismissed share sheets need no handling */ }
  }
  ...
  return <SafeAreaView style={[styles.screen, { backgroundColor: colors.paper }]}><PaperPattern/><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={subjectTheme.accent}/>}>
    ...
    {todayMission?.rest ? <View style={[styles.hero, { backgroundColor: colors.raised, borderColor: colors.ink, borderLeftColor: colors.positive }]}>
      <View style={styles.heroMeta}><Text style={[styles.mono, { color: colors.positive }]}>REST DAY · PLANNED RECOVERY</Text><Text style={[styles.mono, { color: colors.quiet }]}>NO MISSION</Text></View>
      <Text accessibilityRole="header" style={[styles.heroTitle, { color: colors.ink }]}>Rest day</Text>
      <Text style={[styles.reason, { color: colors.ink }]}>No mission today by your plan. Light retrieval only — reread one mastered note, or take the day fully off.</Text>
    </View> : todayMission ? <View style={[styles.hero, todayMission.topicId ? { backgroundColor: colors.raised, borderColor: colors.ink, borderLeftColor: subjectTheme.accent } : { backgroundColor: colors.raised, borderColor: colors.ink, borderLeftColor: colors.warning }]}>
    ...
    <View style={[styles.summary, { borderColor: colors.line, backgroundColor: colors.raised }]} accessibilityLabel={`Level ${progress.level}, ${progress.xp} XP, ${progress.streak} day streak${progress.streakFreezes ? `, ${progress.streakFreezes} streak freezes banked` : ''}, ${progress.accuracy ?? 'no'} percent accuracy, ${progress.lessons} lessons completed`}>
    ...
    <View><SectionHeader title="Rolling 7-day plan" meta={planState ? `${planDone}/7 DONE · SAVED TO ACCOUNT` : 'SAVED TO ACCOUNT'}/>{planState ? planState.days.map(day => { const done = day.status === 'done'; const startable = !done && !!day.topicId && day.date === todayKey && !day.rest; const row = <View style={[styles.planRow, { backgroundColor: done ? colors.positiveWash : day.rest ? colors.muted : 'transparent', borderLeftColor: done || day.rest ? colors.positive : day.date === todayKey && day.topicId ? subjectTheme.accent : colors.line }]}><Text style={[styles.mono, { color: colors.quiet, width: 92 }]}>{day.label}</Text><View style={{ flex: 1, gap: 3 }}><Text style={[styles.planTask, { color: done || day.rest ? colors.positive : colors.ink }]}>{done ? '✓ ' : ''}{day.task}</Text>{done && day.result && <Text style={{ color: colors.positive, fontSize: 13, fontWeight: '700' }}>{day.result.percent}% scored · {day.result.correctMarks}/{day.result.totalMarks} marks{day.result.xpEarned != null ? ` · +${day.result.xpEarned} XP` : ''}{day.result.weakTopics.length ? ` · weaker: ${day.result.weakTopics.join(', ')}` : ''}</Text>}</View>{done ? <Text style={[styles.mono, { color: colors.positive }]}>DONE</Text> : startable ? <Text style={[styles.mono, { color: subjectTheme.accent }]}>START ›</Text> : <Text style={[styles.mono, { color: colors.quiet }]}>{day.rest ? 'REST' : `${day.minutes} MIN`}</Text>}</View>; return startable ? <Pressable key={day.date} accessibilityRole="button" accessibilityLabel={`Start today's mission: ${day.task}`} onPress={() => { const next = startMission(planState!, day.date, day.topicId); setPlanState(next); personalClient.savePlan(next).catch(() => setPlanError('Plan could not be saved. Check your connection and try again.')); router.push(`/lesson/${encodeURIComponent(day.topicId!)}` as never); }} style={{ borderBottomWidth: 1, borderBottomColor: colors.line }}>{row}</Pressable> : <View key={day.date} style={{ borderBottomWidth: 1, borderBottomColor: colors.line }}>{row}</View>; }) : <Text style={{ color: colors.quiet }}>{planError || (planLoaded ? 'Your plan appears once course topics are available.' : 'Reading your saved plan…')}</Text>}</View>
    <Button variant="secondary" onPress={()=>router.push('/notebook')}>MISTAKE NOTEBOOK / {dueCount} DUE{fadedCount ? ` · ${fadedCount} FADING` : ''}</Button><Button variant="secondary" onPress={()=>router.push('/weekly-summary')}>WEEKLY SUMMARY</Button>
    {fixupIds.length > 0 && <Button onPress={() => { new ApiClient(subject).trackEvent('fixup_start', {}); router.push({ pathname: '/practice', params: { fixup: '1', targets: fixupIds.join(',') } }); }}>FIX-UP 5 · WEAK SPOTS</Button>}
    {(reachedMilestones.length > 0 || nextMilestone) && <View><SectionHeader title="Milestones" meta={reachedMilestones.length ? `${reachedMilestones.length} EARNED` : 'FIRST SEAL AWAITS'}/>{reachedMilestones.slice(-3).map(m => <View key={m.id} style={[styles.planRow, { borderLeftColor: subjectTheme.accent }]}><Text style={[styles.mono, { color: subjectTheme.accent }]}>★</Text><View style={{ flex: 1, gap: 2 }}><Text style={[styles.planTask, { color: colors.ink }]}>{m.label}</Text><Text style={{ color: colors.quiet }}>{m.detail}</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`Share ${m.label}`} onPress={() => void shareMilestone(m.id, m.label)} style={{ minHeight: 44, justifyContent: 'center', paddingHorizontal: 12, borderWidth: 1, borderColor: colors.strong }}><Text style={[styles.mono, { color: colors.ink }]}>SHARE</Text></Pressable></View>)}{nextMilestone && <Text style={{ color: colors.quiet }}>Next seal: {nextMilestone.label} — {nextMilestone.value}/{nextMilestone.at}.</Text>}</View>}
