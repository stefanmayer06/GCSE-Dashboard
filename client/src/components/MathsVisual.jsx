    ...
    <figure className={`maths-visual ${compact ? 'compact' : ''} ${expanded ? 'expanded' : ''}`}>
      ...
      <figcaption>
        ...
        <span className="visual-controls">
          <button type="button" className={`visual-control ${detail ? 'on' : ''}`} onClick={() => setDetail((value) => !value)}>
...
function CoordinateVisual({ data, grid, selected, setSelected }) {
  ...
  const xs = all.map((p) => p.x).concat(data.queryX ?? []);
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={data.alt}>
      ...
      {(data.segments || []).map((segment, i) => <line key={i} className="visual-line" x1={sx(segment[0].x)} y1={sy(segment[0].y)} x2={sx(segment[1].x)} y2={sy(segment[1].y)} />)}
      ...
      {points.map((point, i) => {
        ...
        return (
          <g key={`${point.x}-${point.y}-${i}`} className={`visual-point ${active ? 'active' : ''}`} role="button" tabIndex="0" aria-label={pointLabel} onClick={() => setSelected(active ? null : pointLabel)} onKeyDown={(event) => {
...
function regularPolygon(sides, cx = 210, cy = 140, radius = 92) {
  return Array.from({ length: Math.max(3, Math.min(20, sides || 5)) }, (_, i) => {
...
function ShapeVisual({ data, highlighted }) {
  const cls = `visual-shape ${highlighted ? 'highlighted' : ''}`;
  ...
  switch (data.kind) {
    ...
    case 'circle': case 'wheel': drawing = <><circle cx="210" cy="140" r="92" />{data.measure !== 'circumference' && <line className="visual-helper" x1={data.measure === 'radius' ? 210 : 118} y1="140" x2="302" y2="140" />}{data.kind === 'wheel' && Array.from({ length: 8 }, (_, i) => <line key={i} className="visual-helper" x1="210" y1="140" x2={210 + Math.cos(i * Math.PI / 4) * 92} y2={140 + Math.sin(i * Math.PI / 4) * 92} />)}</>; break;
    ...
    case 'bearing': {
      ...
      const ex = 210 + Math.cos(angle) * 95;
      const ey = 145 + Math.sin(angle) * 95;
      drawing = <><circle cx="210" cy="145" r="100" /><line className="visual-helper" x1="210" y1="235" x2="210" y2="38" /><line className="visual-bearing" x1="210" y1="145" x2={ex} y2={ey} /><text x="210" y="26">N</text><text x="210" y="255">S</text><text x="370" y="150">E</text><text x="50" y="150">W</text></>;
      break;
    }
    default: drawing = <polygon points="80,210 340,210 210,55" />;
  }
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={data.alt} className={cls}>
      <g className="visual-shape-lines">{drawing}</g>
      {(data.labels || []).map((item, i) => <text key={i} className="visual-measure" x={item.x} y={item.y}>{item.text}</text>)}
...
function TableVisual({ data }) {
  return (
    <table className="visual-table" aria-label={data.alt}>
      <thead><tr>{data.headings.map((heading, i) => <th key={i}>{heading}</th>)}</tr></thead>
      <tbody>{data.rows.map((row, i) => <tr key={i}>{row.map((cell, j) => j === 0 ? <th key={j}>{cell}</th> : <td key={j}>{cell}</td>)}</tr>)}</tbody>
    </table>
  );
}
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={`Tally chart with ${count} marks.`} className={highlighted ? 'highlighted' : ''}>
      {Array.from({ length: bundles }, (_, group) => {
        const x = 55 + group * 95;
        return <g key={group} className="tally-group">{[0, 1, 2, 3].map((i) => <line key={i} x1={x + i * 16} y1="80" x2={x + i * 16} y2="190" />)}<line x1={x - 8} y1="175" x2={x + 58} y2="94" /></g>;
      })}
      <g className="tally-group">{Array.from({ length: remainder }, (_, i) => <line key={i} x1={55 + bundles * 95 + i * 16} y1="80" x2={55 + bundles * 95 + i * 16} y2="190" />)}</g>
    </svg>
  );
}

function PictogramVisual({ data, highlighted }) {
  return <div className={`pictogram ${highlighted ? 'highlighted' : ''}`} role="img" aria-label={data.alt}><strong>Key: {data.symbol} = {data.value}</strong><div>{Array.from({ length: data.count }, (_, i) => <span key={i}>{data.symbol}</span>)}</div></div>;
}
...
function NumberLineVisual({ data }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={data.alt}>
      ...
      {Array.from({ length: max - min + 1 }, (_, i) => min + i).map((value) => <g key={value}><line className="number-tick" x1={sx(value)} y1="125" x2={sx(value)} y2="145" /><text className="visual-tick" x={sx(value)} y="165">{value}</text></g>)}
...
function CountersVisual({ data, setSelected }) {
  return <div className="counter-bag" role="img" aria-label={data.alt}>{data.groups.map((group) => <button type="button" key={group.label} className="counter-group" onClick={() => setSelected(`${group.label}: ${group.count}`)}><span className="counter-dots">{Array.from({ length: group.count }, (_, i) => <i key={i} style={{ background: group.color }} />)}</span><span>{group.label}</span></button>)}</div>;
}

function DiceVisual() {
  return <div className="dice-row" role="img" aria-label="The six equally likely outcomes on a fair die.">{[1, 2, 3, 4, 5, 6].map((n) => <span key={n} aria-label={`${n}`}>{n}</span>)}</div>;
}
...
function VennVisual({ data, highlighted }) {
  return <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={data.alt} className={highlighted ? 'highlighted' : ''}>{data.events.map((event, i) => <g key={event.label}><circle className="venn-circle" cx={data.disjoint ? 130 + i * 160 : 175 + i * 70} cy="130" r="76" /><text className="visual-measure" x={130 + i * 160} y="120">{event.label}</text><text className="visual-measure secondary" x={130 + i * 160} y="148">P = {event.value}</text></g>)}</svg>;
}

function TreeVisual({ data, highlighted }) {
  return <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={data.alt} className={highlighted ? 'highlighted' : ''}><circle className="visual-node" cx="55" cy="130" r="6" />{(data.stages[0] || []).map((item, i) => <g key={item}><line className="tree-line" x1="55" y1="130" x2="190" y2={75 + i * 110} /><text className="tree-label" x="115" y={88 + i * 88}>{item}</text>{(data.stages[1] || []).map((second, j) => <g key={second}><line className="tree-line second" x1="190" y1={75 + i * 110} x2="350" y2={45 + i * 90 + j * 55} /><text className="tree-label" x="285" y={58 + i * 90 + j * 55}>{second}</text></g>)}</g>)}</svg>;
}

function SampleSpaceVisual({ size, highlighted, setSelected }) {
  return <div className={`sample-space ${highlighted ? 'highlighted' : ''}`} role="grid" aria-label={`Sample-space grid for two ${size}-sided dice.`}>{Array.from({ length: size }, (_, row) => Array.from({ length: size }, (_, col) => <button type="button" role="gridcell" key={`${row}-${col}`} onClick={() => setSelected(`Outcome (${row + 1}, ${col + 1}), total ${row + col + 2}`)}>{row + 1},{col + 1}</button>))}</div>;
}
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Growing cross patterns containing five, nine and thirteen dots." className={highlighted ? 'highlighted' : ''}>
      {patterns.map((arm, patternIndex) => {
        ...
        return <g key={arm} className="dot-pattern">{points.map(([x, y], i) => <circle key={i} cx={x} cy={y} r="6" />)}<text x={cx} y="220">Pattern {patternIndex + 1}</text></g>;
      })}
    </svg>
  );
}
