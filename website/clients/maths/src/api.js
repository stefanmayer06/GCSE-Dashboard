...
const eventsBase = '/api';
export const SUBJECT = window.location.pathname.startsWith('/maths-higher') ? 'maths-higher' : 'maths';
...
async function eventReq(path, opts = {}) {
  const token = (await supabaseAccessToken()) || storedSupabaseAccessToken();
  const res = await fetch(eventsBase + path, {
    headers: {
      'Content-Type': 'application/json',
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
    },
...
export const api = {
  health: () => req('/health'),
  track: (name, metadata) => eventReq('/events', { method: 'POST', body: { name, subject: SUBJECT, metadata } }).catch(() => undefined),
  topics: () => req('/topics'),
