...
test('auth uses durable hashed records and stable OAuth identities', async (t) => {
  ...
  response = await fetch(`${appUrl}/api/auth/account`, {
    method: 'DELETE',
    headers: { Cookie: adminCookie, 'Content-Type': 'application/json' },
    body: JSON.stringify({ confirmation: 'delete', userId: 'someone-else' }),
  });
  assert.equal(response.status, 400);
  assert.deepEqual(await response.json(), {
    error: 'Set confirmation to DELETE to delete your account.',
  });
  ...
  assert.deepEqual(await response.json(), {
    error: 'Account deletion is only available with Supabase Auth.',
  });
