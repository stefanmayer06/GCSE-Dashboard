...
test('Supabase finalization submits the server operation without a stale state callback', async () => {
  let submitted;
  const storage = {
    driver: 'supabase',
    finalizeStudySession(criteria, operation) {
      submitted = { criteria, operation };
      return Promise.resolve({ status: 'completed', result: operation.response });
    },
  };
  const db = createDb('supabase-user', 'english', storage);
  const operation = {
    practiceRecords: { topicId: 'language', correct: 1, total: 2 },
    scoreXp: 3,
    response: { totalMarks: 2 },
  };

  const finalized = await db.finalizeStudySession({ id: 'session-id', kind: 'practice' }, operation);
